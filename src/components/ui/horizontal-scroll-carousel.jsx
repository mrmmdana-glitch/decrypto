...
const BgIllicitFlow = () => (
  <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%", opacity:0.07, pointerEvents:"none" }}
    ...
    <style>{`
      @keyframes flowDash1 { from { stroke-dashoffset: 0 } to { stroke-dashoffset: -180 } }
      @keyframes flowDash2 { from { stroke-dashoffset: 0 } to { stroke-dashoffset: -220 } }
      @keyframes flowDash3 { from { stroke-dashoffset: 0 } to { stroke-dashoffset: -160 } }
    `}</style>
    {/* Edge nodes */}
...
const BgLiquidity = () => (
  <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%", opacity:0.08, pointerEvents:"none" }}
    viewBox="0 0 480 340" preserveAspectRatio="xMidYMid slice">
    <style>{`@keyframes liqFlow { from{stroke-dashoffset:0} to{stroke-dashoffset:-240} }`}</style>
...
const HorizontalScrollCarousel = ({ items, scrollContainer }) => {
  ...
  // Scroll progress mapped so [0, 0.85] drives the pan and [0.85, 1.0] holds
  // the last card in view — giving the user time to read it before scrolling on
  const x = useTransform(scrollYProgress, [0, 0.85], ["2%", "-84%"]);
  const [hoveredId, setHoveredId] = useState(null);

  return (
    <section ref={targetRef} style={{ height: "600vh", background: "#08090e", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col justify-center">
        <div style={{ padding: "0 64px 28px", textAlign: "center" }}>
          <span style={{ fontSize: "1.75rem", fontWeight: 800, letterSpacing: "0.12em", color: "#ef4444", textTransform: "uppercase" }}>
            What's the problem?
          </span>
        </div>
          ...
          <motion.div style={{ x }} className="flex gap-10 pl-16">
            {items.map(item => (
              <ProblemCard
                key={item.id}
                item={item}
                state={hoveredId === null ? "default" : hoveredId === item.id ? "active" : "dim"}
